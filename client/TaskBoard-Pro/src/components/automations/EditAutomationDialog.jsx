...
import { updateAutomation, getProject, getProjectMembers } from '../../services/api';

const EditAutomationDialog = ({ open, onClose, automation, onAutomationUpdated }) => {
  ...
  useEffect(() => {
    if (open && automation) {
      // Set form values from automation data
      ...
      setActionParams(automation.action?.params || {});
      
      // Fetch project data
      if (automation.project) {
        fetchProjectData(automation.project);
      }
    }
  }, [open, automation]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateAutomation(automation._id, {
        name,
        trigger: {
          type: triggerType,
      ...
      if (onAutomationUpdated) {
        onAutomationUpdated(response.data);
      }
